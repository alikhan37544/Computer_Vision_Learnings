...
class AIGymWithCamera:
    def __init__(self):
        # Initialize AIGym
        self.gym = AIGym()

        # Set parameters (if needed)
        self.kpts_to_check = [0, 1, 2]  # Example keypoints to check
        self.line_thickness = 2
        self.view_img = True  # Set to True to display the video stream
        self.pose_up_angle = 145.0
        self.pose_down_angle = 90.0
        self.pose_type = "pushup"

        self.gym.set_args(
            self.kpts_to_check, self.line_thickness, self.view_img,
            self.pose_up_angle, self.pose_down_angle, self.pose_type
        )


    def start(self):
        # Open video capture
        # 0 for default camera, you can change it to another index if you have multiple cameras
        ...
        while True:
            ...
            # Dummy pose estimation results (for testing purposes)
            # Replace DummyPoseEstimationData with your actual pose estimation data
            dummy_results = [[DummyPoseEstimationData()]]

            # Start counting gym steps
            result_frame = self.gym.start_counting(
                frame, dummy_results, frame_count)
...
# Create an instance of AIGymWithCamera and start counting gym steps
if __name__ == "__main__":
    gym_with_camera = AIGymWithCamera()
    gym_with_camera.start()
